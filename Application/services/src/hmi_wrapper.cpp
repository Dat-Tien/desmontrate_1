...
HmiWrapper& HmiWrapper::GetInstance()
{
    static HmiWrapper instance;
    return instance;
}

HmiWrapper::HmiWrapper() = default;

HmiWrapper::~HmiWrapper() {
    Stop();
}

void HmiWrapper::RegisterCallback(Callback callback) {
    m_callback = std::move(callback);
}

bool HmiWrapper::Connect(const std::string& socket_path) {
    m_socket_path = socket_path;
    m_socket_fd = socket(AF_UNIX, SOCK_STREAM, 0);
    if (m_socket_fd < 0) {
        std::cerr << "[HmiWrapper] Failed to create socket: " << std::strerror(errno) << std::endl;
        return false;
    }
...
void HmiWrapper::StartListening() {
    if (m_socket_fd < 0 || m_running) {
        return;
    }

    m_running = true;
    m_listener_thread = std::thread(&HmiWrapper::ListenLoop, this);
}

void HmiWrapper::Stop() {
