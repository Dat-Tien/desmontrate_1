...
constexpr const char* kPowerAppSocket = "/tmp/_power_service.sock";
constexpr const char* kPowerCmdSocket = "/tmp/_power_service_cmd.sock";
...
int CreateServerSocket(const char* socket_path) {
    ...
    if (server_fd < 0) {
        std::cerr << "[PowerService] Failed to create socket: " << socket_path << std::endl;
        return -1;
    }
    ...
    if (bind(server_fd, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0) {
        std::cerr << "[PowerService] Bind failed: " << socket_path << std::endl;
        close(server_fd);
        return -1;
    }

    if (listen(server_fd, 5) < 0) {
        std::cerr << "[PowerService] Listen failed: " << socket_path << std::endl;
        close(server_fd);
        unlink(socket_path);
        return -1;
    }
...
bool ForwardLine(int app_client_fd, const std::string& line) {
    if (app_client_fd < 0) {
        std::cerr << "[PowerService] No Application client connected" << std::endl;
        return false;
    }
    ...
    if (written <= 0) {
        std::cerr << "[PowerService] Failed to forward message to Application" << std::endl;
        return false;
    }

    std::cout << "[PowerService] Forwarded to Application: " << line << std::endl;
    return true;
}
...
int main() {
    ...
    const int app_server_fd = CreateServerSocket(kPowerAppSocket);
    const int cmd_server_fd = CreateServerSocket(kPowerCmdSocket);

    if (app_server_fd < 0 || cmd_server_fd < 0) {
        if (app_server_fd >= 0) close(app_server_fd);
        if (cmd_server_fd >= 0) close(cmd_server_fd);
        unlink(kPowerAppSocket);
        unlink(kPowerCmdSocket);
        return 1;
    }

    std::cout << "[PowerService] Waiting for Application on " << kPowerAppSocket << std::endl;
    const int app_client_fd = accept(app_server_fd, nullptr, nullptr);
    if (app_client_fd < 0) {
        std::cerr << "[PowerService] Failed to accept Application client" << std::endl;
        close(app_server_fd);
        close(cmd_server_fd);
        unlink(kPowerAppSocket);
        unlink(kPowerCmdSocket);
    ...
    while (g_running) {
        ...
        if (ret < 0) {
            ...
            std::cerr << "[PowerService] poll failed" << std::endl;
            continue;
        }
    ...
    std::cout << "[PowerService] Shutdown" << std::endl;
    return 0;
} 